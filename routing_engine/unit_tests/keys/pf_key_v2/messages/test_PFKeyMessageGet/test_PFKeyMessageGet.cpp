...
TEST(test_PFKeyMessageGet, test_Deserialize)
{
	...
	// Compare header data
	ASSERT_EQ(SADB_GET, msg.GetMessageType());
...
TEST(test_PFKeyMessageGet, test_Serialize)
{
	std::stringstream sstream;
	Logger::SetLogLevel(LOG_VERBOSE);
	Logger::SetLogStdOut(true);
	...
	// Compare header data
	ASSERT_EQ(SADB_GET, msg2.GetMessageType());
